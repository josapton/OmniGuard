...
The OmniGuard REST API lets you integrate security scanning into your existing workflows — CI/CD pipelines, automation scripts, SIEMs, or custom dashboards. Generate an API key from the **Settings → API Keys** tab, then use it to trigger scans and retrieve results without the web UI.
...
All keys are hashed with SHA-256 — OmniGuard never stores raw keys. Each key is granted `scan:create`, `scan:read`, and `findings:read` permissions by default.
...
<p align="center"><em>Figure 1 — OmniGuard System Architecture Overview</em></p>

- **Client Layer** — A React SPA (built with Vite + TypeScript) served by Nginx. The Global Search command palette (⌘K) queries Elasticsearch for cross-scan full-text search.
- **Docker Ecosystem** — A Python FastAPI backend handles all compute: scan orchestration, AI threat reports, interactive AI chat, domain policy evaluation, and Elasticsearch syncing. PostgreSQL stores all persistent data locally.
- **External Services** — Three external platforms are consumed: Firecrawl for web scraping, Google Gemini/Groq for AI inference, and Elastic Cloud for enterprise-grade full-text search.
- **Data Flow** — A user enters a domain → the policy agent evaluates it → Firecrawl scrapes and maps the site → data is parsed and findings generated → risk score calculated → results stored in PostgreSQL → automatically synced to Elasticsearch.
...
- API Keys for Firecrawl, Supabase, Groq/Gemini, and Google Cloud (depending on active modules).

### Steps

1. **Clone the repository**
   ```bash
   git clone https://github.com/josapton/OmniGuard.git
   cd OmniGuard
   ```
