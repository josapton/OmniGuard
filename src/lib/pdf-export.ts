...
export function exportReportAsPdf(scan: Scan, findings: Finding[], surfaceInsights?: Record<string, string>) {
  ...
  const addFooter = (pageNum: number) => {
    ...
    doc.text(`OmniGuard — Confidential | Page ${pageNum} of [TOTAL]`, margin, pageHeight - 8);
  ...
  doc.text("Generated by OmniGuard Automated Threat Intelligence Platform", margin, pageHeight - 15);
  ...
  for (let i = 2; i <= totalPages; i++) {
    ...
    doc.setFontSize(7);
    doc.setTextColor(140);
    doc.text(`OmniGuard — Confidential | Page ${i - 1} of ${totalPages - 1}`, margin, pageHeight - 8);
    doc.text(`Report ID: TL-${scan.id.substring(0, 8).toUpperCase()}`, pageWidth - margin, pageHeight - 8, { align: "right" });
  }

  const fileName = `OmniGuard_Report_${scan.domain.replace(/[^a-zA-Z0-9]/g, "_")}_${new Date(scan.created_at).toISOString().split("T")[0]}.pdf`;
  doc.save(fileName);
}
